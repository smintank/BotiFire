...
async def send_menu(message: Message) -> None:
    ...
    await message.answer("Что вы хотите сделать?", reply_markup=markup.main_inline_menu)
...
async def process_callback_notify(callback: CallbackQuery) -> None:
    ...
    await callback.answer('Ведите фамилию сотрудника:', reply_markup=markup.remove_menu)
...
async def process_callback_posts(callback: CallbackQuery) -> None:
    ...
    await callback.answer(f'Нажата кнопка "{post}"', reply_markup=markup.remove_menu)
    await callback.answer('Ведите фамилию следующего сотрудника:', reply_markup=markup.remove_menu)
...
async def process_callback_ok(callback: CallbackQuery):
    ...
    await callback.answer('✅🪖🔫', reply_markup=markup.remove_menu)
...
async def process_callback_deny(callback: CallbackQuery):
    ...
    await callback.answer('Укажите причину:', reply_markup=markup.remove_menu)
